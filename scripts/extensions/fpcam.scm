...
(define (fpcam:update)
  ...
  (let* ((main-camera (camera:main))
    ...
    (when (key:down? keys/A)
      (camera:move main-camera direction/LEFT move-speed))
    (when (key:down? keys/E)
      (camera:move main-camera direction/RIGHT move-speed))
    (when (key:down? keys/COMMA)
      (camera:move main-camera direction/FORWARD move-speed))
    (when (key:down? keys/O)
      (camera:move main-camera direction/BACKWARD move-speed))))
