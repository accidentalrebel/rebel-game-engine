...
sexp sexp_sprite_draw_stub (sexp ctx, sexp self, sexp_sint_t n, sexp arg0, sexp arg1, sexp arg2, sexp arg3, sexp arg4, sexp arg5) {
  sexp res;
  ...
  if (! ((sexp_pointerp(arg5) && (sexp_pointer_tag(arg5) == sexp_unbox_fixnum(sexp_vector_ref(sexp_opcode_argn_type(self), SEXP_TWO)))) || sexp_not(arg5)))
    return sexp_type_exception(ctx, self, sexp_unbox_fixnum(sexp_vector_ref(sexp_opcode_argn_type(self), SEXP_TWO)), arg5);
  res = ((sprite::Draw((struct Sprite*)sexp_cpointer_value(arg0), *(struct Vec3*)sexp_cpointer_value(arg1), sexp_flonum_value(arg2), sexp_flonum_value(arg3), *(struct Vec3*)sexp_cpointer_value(arg4), (struct Shader*)sexp_cpointer_maybe_null_value(arg5))), SEXP_VOID);
  return res;
}

sexp sexp_shader_create_stub (sexp ctx, sexp self, sexp_sint_t n, sexp arg0, sexp arg1) {
  struct Shader struct_res;
  struct Shader* ptr_res;
  sexp res;
  if (! sexp_stringp(arg0))
    return sexp_type_exception(ctx, self, SEXP_STRING, arg0);
  if (! sexp_stringp(arg1))
    return sexp_type_exception(ctx, self, SEXP_STRING, arg1);
  struct_res = shader::Create(sexp_string_data(arg0), sexp_string_data(arg1));
  ptr_res = (struct Shader*) malloc(sizeof(struct Shader));
  memcpy(ptr_res, &struct_res, sizeof(struct Shader));
...
sexp sexp_FreeShader_stub (sexp ctx, sexp self, sexp_sint_t n, sexp x) {
...
sexp sexp_init_library (sexp ctx, sexp self, sexp_sint_t n, sexp env, const char* version, const sexp_abi_identifier_t abi) {
  ...
  name = sexp_c_string(ctx, "Shader", -1);
  sexp_Shader_type_obj = sexp_register_c_type(ctx, name, sexp_FreeShader_stub);
  tmp = sexp_string_to_symbol(ctx, name);
  sexp_env_define(ctx, env, tmp, sexp_Shader_type_obj);
  sexp_type_slots(sexp_Shader_type_obj) = SEXP_NULL;
  sexp_push(ctx, sexp_type_slots(sexp_Shader_type_obj), sexp_intern(ctx, "id", -1));
  sexp_type_getters(sexp_Shader_type_obj) = sexp_make_vector(ctx, SEXP_ONE, SEXP_FALSE);
  sexp_type_setters(sexp_Shader_type_obj) = sexp_make_vector(ctx, SEXP_ONE, SEXP_FALSE);
  tmp = sexp_make_type_predicate(ctx, name, sexp_Shader_type_obj);
  name = sexp_intern(ctx, "shader?", 7);
  ...
  op = sexp_define_foreign(ctx, env, "FreeShader", 1, sexp_FreeShader_stub);
  if (sexp_opcodep(op)) {
    sexp_opcode_return_type(op) = SEXP_VOID;
    sexp_opcode_arg1_type(op) = sexp_make_fixnum(sexp_type_tag(sexp_Shader_type_obj));
  }
  ...
  op = sexp_define_foreign(ctx, env, "sprite-draw", 6, sexp_sprite_draw_stub);
  if (sexp_opcodep(op)) {
    sexp_opcode_return_type(op) = SEXP_VOID;
    sexp_opcode_arg1_type(op) = sexp_make_fixnum(sexp_type_tag(sexp_Sprite_type_obj));
    ...
    sexp_vector_set(sexp_opcode_argn_type(op), SEXP_TWO, sexp_make_fixnum(sexp_type_tag(sexp_Shader_type_obj)));
  }
  op = sexp_define_foreign(ctx, env, "shader-create", 2, sexp_shader_create_stub);
  if (sexp_opcodep(op)) {
    sexp_opcode_return_type(op) = sexp_make_fixnum(sexp_type_tag(sexp_Shader_type_obj));
